	...
	function addCharacterUI() {
		...
		var reloadImage:FlxButton = new FlxButton(imageInputText.x + 210, imageInputText.y - 3, "Reload Image", function()
		{
			...
			if(char.animation.curAnim != null) {
				char.playAnim(char.animation.curAnim.name, 0, true);
			}
		});
			...
			for (anim in char.animationsArray) {
				if(animationInputText.text == anim.anim) {
					...
					if(resetAnim && char.animationsArray.length > 0) {
						char.playAnim(char.animationsArray[0].anim, 0, true);
					}
					reloadAnimationDropDown();
					genBoyOffsets();
	...
	override function update(elapsed:Float)
	{
		...
		if(!charDropDown.dropPanel.visible) {
			...
			if(char.animationsArray.length > 0) {
				...
				{
					char.playAnim(char.animationsArray[curAnim].anim, 0, true);
					genBoyOffsets();
				}
				...
				for (i in 0...controlArray.length) {
					if(controlArray[i]) {
						...
						char.playAnim(char.animationsArray[curAnim].anim, 0, false);
						genBoyOffsets();
					}
				}
			}
